 ...
 *
 * 1) NHÁP (DRAFT) — dù nội bộ hay ngoài ngành: CHỈ người tạo và ADMIN thấy
 *    được (+ người được TAG trong meeting_participants nếu có). BGD KHÔNG
 *    tự động thấy draft — vì cuộc họp nháp "chưa có hiệu lực".
 * 2) Đã DUYỆT (status <> DRAFT) + họp NỘI BỘ: BGD thấy toàn bộ (quyền giám
 *    sát cuộc họp đã có hiệu lực), hoặc phòng chủ trì, hoặc phòng đã được
...
export function canViewMeeting(
  ...
  if (participants.some((p) => p.user_id === u.id)) return true;
 ...
 * QUY TẮC: chỉ ADMIN mới được xoá, và CHỈ khi cuộc họp còn ở trạng thái NHÁP (DRAFT).
...
/**
 * Xoá cuộc họp dành riêng cho THƯ KÝ — khác với canDeleteMeeting (chỉ ADMIN, chỉ
 * lúc Nháp) ở chỗ: Thư ký được xoá HẲN cuộc họp do mình quản lý (người tạo, hoặc
 * cùng phòng Ban Giám đốc chủ trì — xem canManageMeeting) ở MỌI trạng thái, kể cả
 * sau khi đã Duyệt/đang diễn ra/đã đóng. Chỉ khoá lại khi cuộc họp đã Lưu trữ
 * (ARCHIVED) — giữ nguyên hồ sơ khi không còn hiệu lực.
 */
export function canDeleteMeetingAsThuky(meeting: Meeting, u?: Profile | null) {
  if (!u || u.role !== 'THUKY') return false;
 ...
 */
/**
 * QUY TẮC: người tạo/chủ trì phòng họp được SỬA (thêm version tài liệu) hoặc
 * XOÁ nội dung đã có (tài liệu, ý kiến, tệp đính kèm) ở MỌI trạng thái cuộc họp —
 * kể cả sau khi đã Duyệt (không còn giới hạn chỉ lúc còn Nháp). Chỉ bị khoá lại
 * khi cuộc họp đã chuyển sang Lưu trữ (ARCHIVED). ADMIN luôn được phép.
 */
export function canEditMeetingContent(meeting: Meeting, u?: Profile | null) {
  if (!u) return false;
  if (isAdmin(u)) return true;
  if (meeting.status === 'ARCHIVED') return false;
  return isMeetingCreator(meeting, u);
}
...
/**
 * Xoá hẳn một tài liệu (kèm mọi version) đã đăng.
 * - Admin hoặc người tạo cuộc họp (khi còn Nháp): xem canEditMeetingContent.
 * - Ngoài ra, CHÍNH người đã tải tài liệu đó lên luôn được tự xoá tài liệu của
 *   mình (kể cả sau khi cuộc họp đã được duyệt) — để họ tự sửa khi lỡ đẩy
 *   nhầm file, không phải chờ thư ký/admin xử lý hộ. Chỉ áp dụng khi cuộc họp
 *   chưa bị Huỷ/Lưu trữ.
 */
export function canDeleteDocument(meeting: Meeting, doc: DocumentRow, u?: Profile | null) {
  if (canEditMeetingContent(meeting, u)) return true;
  if (!u) return false;
  if (meeting.status === 'ARCHIVED') return false;
  return doc.uploaded_by === u.id;
}
